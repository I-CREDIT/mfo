...
class Login extends React.Component {
  ...
  async handleSubmit(values) {
    ...
    if (
      ...
      axios
        .get(
          `https://api.i-credit.kz/api/checkPerson?phone=${replaceDate(
            values.phone
          )}&iin=${values.iin}`
        )
        .then((response) => {
          ...
          if (response.data.success) {
            this.setState({
              errorMessage: null,
              message:
                "Вам было отправлено СМС со ссылкой. Пожалуйста, проверьте и перейдите по ссылке для восстановление пароля",
            });
          }
          // Router.push('/login')
        })
                ...
                <nav aria-label="container breadcrumb">
                  <ol className="breadcrumb text-center">
                    <li className="breadcrumb-item ">
                      <AppLink className="br-item" href="/login">
                        Личный кабинет
                      </AppLink>
                    </li>
