...
const ResetPasword = () => {
  ...
  const handleSubmit = values => {
    ...
    if(values.password.length>4&&values.password !== values.passwordConfirm) {
      ...
      axios.get(`https://api.money-men.kz/api/resetPassword?phone=${phone}&iin=${iin}&password=${values.password}`)
        .then(res=> {
          console.log(res)
          if(res.data.success) {
                    ...
                    <div className="buttonForm">
                      {btnLoading === true
                        ? <Spinner
                            ...
                            spinnerWidth={2}
                            visible={true}/>
                        : <button className="loginbutton" type="submit">Подтвердить</button>}
                    </div>
                  </Form>
                )}
              </Formik>
