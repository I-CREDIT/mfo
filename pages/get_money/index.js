...
import CodeConfirmBMG from "../../components/registrationForm/CodeConfirmBMG";
...
const mapDispatchToProps = (dispatch) => ({
  stepRegistration: (step) => {
    dispatch(stepRegistration(step));
  },
  resetRegistration: () => {
    dispatch(actions.reset("registration"));
  },
  resetRegistration2: () => {
    dispatch(actions.reset("registration2"));
  },
  resetRegistration3: () => {
    dispatch(actions.reset("registration3"));
  },
  emptyMessage: () => {
    ...
    return (
      <React.Fragment>
        <React.Fragment>
          ...
          <Head>
            <title>Деньги в кредит</title>
          </Head>
          <Stepper
            className="otherPages"
            activeStep={
              this.state.registrationStep?.toString() === "4" ||
              this.props.stepregistration.toString() === "4"
                ? 1
                : this.state.registrationStep > this.props.stepregistration &&
                  this.state.registrationStep > 1
                ? parseInt(this.state.registrationStep) - 1
                : parseInt(stepcheck(this.props.stepregistration))
            }
          ...
          {this.state.registrationStep > this.props.stepregistration &&
          this.state.registrationStep > 1
            ? handleSteps(parseInt(this.state.registrationStep))
            : handleSteps(parseInt(this.props.stepregistration))}
        </React.Fragment>
      </React.Fragment>
    );
  }
