...
class Aggrement extends React.Component {
  ...
  async getUserDocument(token) {
    ...
    if(getUrlParameter('token').length !== 0) {
      await axios.get(`http://185.233.3.138/api/newGetData?token=${token}`)
      .then((response) => {
      ...
      await axios.get(`http://185.233.3.138/api/newGetData?leadID=${token}`)
      .then((response) => {
        console.log(response)
        if(response.data.success) {
          this.setState({
  ...
  async sendAgreementStatus(){
    ...
    try{
       axios.get('http://185.233.3.138/api/agreementNew',{ params:{
